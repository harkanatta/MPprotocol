...
DF$`Organic material` <- DF$Frozen-DF$Flask
DF <- DF %>% mutate_if(is.numeric, ~round(., 3))
par(mfrow=c(1,2))
plot((DF$result-DF$PDandFilters)~DF$Treatment,xlab='Treatment',ylab = 'Organic material')
plot((DF$result-DF$PDandFilters)~DF$Sample,xlab='Samples',ylab = 'Organic material')

DF$Residual <- ((DF$result-DF$PDandFilters))
DF$`Digestion Efficiency` <- round(1-DF$Residual/DF$`Organic material`,2)



#DF <- DF[c(6,2,7),]
knitr::kable(DF[,c(2,6,9,11)],row.names = F)
